...
export default function OverviewPage() {
  ...
  return <>
    ...
    <section className="next-actions" aria-labelledby="next-actions-title"><div><span className="overline">ادامهٔ مسیر</span><h3 id="next-actions-title">اقدام بعدی را انتخاب کنید</h3></div><div className="action-links"><Link href={`${base}/imports`}><Icon name="upload" /><span><strong>ورود داده مالی</strong><small>بارگذاری امن CSV و XLSX</small></span></Link><Link href={`${base}/financial-model`}><Icon name="layers" /><span><strong>تکمیل مدل مالی</strong><small>طبقه‌بندی حساب‌های باقی‌مانده</small></span></Link><Link href={`${base}/analysis`}><Icon name="chart" /><span><strong>تحلیل دوره‌ای</strong><small>محاسبه شاخص‌ها و کنترل پوشش داده</small></span></Link></div></section>
  </>;
}
