...
export function ImportWorkspace({ company, batchId }: { company: Company; batchId: string }) {
  ...
  return <div className="mapping-workspace">
    ...
    <form className="mapping-layout" onSubmit={saveMapping}>
      <section className="mapping-panel" aria-labelledby="mapping-title"><div className="mapping-section-heading"><div><h3 id="mapping-title">معنای ستون‌ها را تأیید کنید</h3><p>پیشنهادها خودکارند، اما فقط با تأیید شما ثبت می‌شوند.</p></div><span>{Object.values(mapping).filter(Boolean).length} از {fields.length}</span></div><div className="mapping-rows">{fields.map((field) => { const required = isRequiredField(field, preview.required_fields, preview.alternative_required_fields); const suggestion = preview.suggestions.find((item) => item.target_field === field); return <label className="mapping-row" key={field}><span className="target-field"><strong>{fieldLabels[field] ?? field}</strong><small>{required ? "الزامی" : "اختیاری"}{suggestion ? ` · پیشنهاد ${new Intl.NumberFormat("fa-IR").format(suggestion.confidence)}٪` : ""}</small></span><Icon name="arrow" /><NativeSelect value={mapping[field] ?? ""} onChange={(event) => setMapping((current) => ({ ...current, [field]: event.target.value }))} disabled={!canEdit}><NativeSelectOption value="">انتخاب نشده</NativeSelectOption>{preview.columns.map((column) => <NativeSelectOption key={column} value={column} disabled={Object.entries(mapping).some(([target, source]) => target !== field && source === column)}>{column}</NativeSelectOption>)}</NativeSelect></label>; })}</div></section>
      <aside className="mapping-settings"><div><h3>قواعد تبدیل</h3><p>تبدیل تاریخ و مبلغ باید صریح و قابل حسابرسی باشد.</p></div><label>تقویم تاریخ‌ها<NativeSelect value={calendar} onChange={(event) => setCalendar(event.target.value as "jalali" | "gregorian")} disabled={!canEdit}><NativeSelectOption value="jalali">هجری شمسی</NativeSelectOption><NativeSelectOption value="gregorian">میلادی</NativeSelectOption></NativeSelect></label><label>واحد مبالغ<NativeSelect value={currencyUnit} onChange={(event) => setCurrencyUnit(event.target.value as "rial" | "toman")} disabled={!canEdit}><NativeSelectOption value="rial">ریال</NativeSelectOption><NativeSelectOption value="toman">تومان — تبدیل به ریال</NativeSelectOption></NativeSelect></label><label>نام الگو <span>(اختیاری)</span><Input value={profileName} onChange={(event) => setProfileName(event.target.value)} disabled={!canEdit} placeholder="مثلاً خروجی نرم‌افزار سپیدار" /></label><div className="mapping-meta"><span>شیت</span><strong>{preview.selected_sheet}</strong><span>ردیف عنوان</span><strong>{new Intl.NumberFormat("fa-IR").format(preview.header_row)}</strong></div>{preview.mapping ? <div className="locked-note"><Icon name="shield" /><span><strong>نگاشت ثبت شده است</strong>نسخهٔ تأییدشده برای حفظ قابلیت حسابرسی تغییر نمی‌کند.</span></div> : <Button className="primary-button" disabled={!mappingComplete || busy !== null}>{busy === "mapping" ? "در حال ثبت…" : "تأیید نگاشت"}</Button>}</aside>
    </form>

    <section className="data-preview" aria-labelledby="preview-title"><div className="mapping-section-heading"><div><h3 id="preview-title">پیش‌نمایش داده</h3><p>نمونه‌ای از فایل اصلی؛ ردیف عنوان در محاسبات وارد نمی‌شود.</p></div><span>{new Intl.NumberFormat("fa-IR").format(preview.rows.length)} ردیف نمونه</span></div><div className="table-scroll"><Table><TableHeader><TableRow><TableHead>ردیف</TableHead>{preview.columns.map((column) => <TableHead key={column}>{column}</TableHead>)}</TableRow></TableHeader><TableBody>{preview.rows.map((row) => <TableRow key={row.row_number}><TableCell>{new Intl.NumberFormat("fa-IR").format(row.row_number)}</TableCell>{preview.columns.map((column) => <TableCell key={column} title={displayValue(row.raw[column])}>{displayValue(row.raw[column])}</TableCell>)}</TableRow>)}</TableBody></Table></div></section>
