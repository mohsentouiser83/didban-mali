        ...
        <div className="flex min-h-screen">
          ...
          <aside className={cn("ds-glass fixed inset-y-0 start-0 z-40 flex flex-col border-e border-[var(--ds-border)] transition-[width,transform] duration-300 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0", collapsed ? "w-[84px]" : "w-[280px]", mobileOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0")}>
            <div className="flex h-20 items-center gap-3 border-b border-[var(--ds-border)] px-5">
              <BrandMark />
              {!collapsed && <div className="min-w-0"><strong className="block truncate text-base font-extrabold">دیدبان مالی</strong><span className="block truncate text-xs text-[var(--ds-foreground-faint)]">Financial Intelligence OS</span></div>}
            </div>
            <nav className="flex-1 space-y-1.5 overflow-y-auto p-3" aria-label="ناوبری اصلی">
              {!collapsed && <p className="px-3 pb-2 pt-3 text-xs font-bold text-[var(--ds-foreground-faint)]">فضای تحلیل</p>}
            ...
            <div className="border-t border-[var(--ds-border)] p-3">
              <Button className={cn("ds-focus flex min-h-12 w-full items-center rounded-xl bg-[var(--ds-muted)] transition-[background-color,transform] duration-200 active:scale-[0.96]", collapsed ? "justify-center" : "gap-3 px-3")}><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--ds-accent-soft)] text-xs font-extrabold text-[var(--ds-accent)]">مت</span>{!collapsed && <span className="min-w-0 text-start"><strong className="block truncate text-sm">مهدی تهرانی</strong><small className="block truncate text-xs text-[var(--ds-foreground-faint)]">مدیر مالی</small></span>}</Button>
          ...
          <div className="min-w-0 flex-1">
            <header className="ds-glass sticky top-0 z-20 flex h-20 items-center gap-3 border-b border-[var(--ds-border)] px-4 md:px-6 xl:px-8">
              ...
              <div className="hidden min-w-0 md:block"><p className="text-xs text-[var(--ds-foreground-faint)]">سیستم طراحی / نمای زنده</p><h1 className="truncate text-base font-extrabold">مرکز کنترل مالی</h1></div>
              <div className="relative me-auto hidden w-full max-w-sm lg:block"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-[var(--ds-foreground-faint)]" /><Input className="ps-10 pe-14" aria-label="جست‌وجوی سراسری" placeholder="جست‌وجوی سند، حساب یا یافته…" /><kbd className="absolute end-3 top-1/2 -translate-y-1/2 rounded-md border border-[var(--ds-border)] px-1.5 py-0.5 text-xs text-[var(--ds-foreground-faint)]">⌘ K</kbd></div>
              <ThemeToggle />
            ...
            <main id="design-content" className="mx-auto max-w-[1500px] space-y-6 p-4 md:p-6 xl:p-8">
              ...
              <section className="grid gap-4 md:grid-cols-3" aria-label="شاخص‌های کلیدی نمونه">{metrics.map((metric) => <Card key={metric.title} className="group overflow-hidden transition-[transform,box-shadow] hover:-translate-y-0.5 hover:shadow-[var(--ds-shadow-md)]"><CardContent className="flex items-start gap-4 pt-5 md:pt-6"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[var(--ds-accent-soft)] text-[var(--ds-accent)]"><metric.icon className="size-5" /></span><div className="min-w-0"><p className="text-sm text-[var(--ds-foreground-soft)]">{metric.title}</p><p className="ds-persian-numerals mt-2 text-2xl font-extrabold tracking-tight">{metric.value}</p><p className="mt-1 text-xs text-[var(--ds-foreground-faint)]">{metric.unit}</p></div><Badge variant={metric.variant} className="ms-auto">{metric.delta}</Badge></CardContent></Card>)}</section>

              <section className="grid gap-4 xl:grid-cols-[1.45fr_.75fr]">
                <Card><CardHeader className="flex-row items-start justify-between"><div><CardTitle>روند جریان نقدی</CardTitle><CardDescription>مقایسهٔ ورودی و خروجی شش ماه اخیر — میلیارد ریال</CardDescription></div><div className="flex gap-3 text-xs text-[var(--ds-foreground-soft)]"><span className="flex items-center gap-1.5"><i className="size-2 rounded-full bg-[var(--ds-primary)]" />ورودی</span><span className="flex items-center gap-1.5"><i className="size-2 rounded-full bg-[var(--ds-accent)]" />خروجی</span></div></CardHeader><CardContent><CashflowChart /></CardContent></Card>
                <Card className="overflow-hidden"><CardHeader><div className="flex items-center justify-between"><span className="grid size-11 place-items-center rounded-xl bg-[var(--ds-accent-soft)] text-[var(--ds-accent)]"><ShieldCheck className="size-5" /></span><Badge variant="success">پایدار</Badge></div><CardTitle className="mt-3">سلامت کنترل‌ها</CardTitle><CardDescription>پوشش کنترل‌های حیاتی این دوره</CardDescription></CardHeader><CardContent className="space-y-5"><div><div className="mb-2 flex justify-between text-sm"><span>کنترل اسناد</span><strong>۹۸٪</strong></div><Progress value={98} /></div><div><div className="mb-2 flex justify-between text-sm"><span>تطبیق بانکی</span><strong>۸۶٪</strong></div><Progress value={86} /></div><div><div className="mb-2 flex justify-between text-sm"><span>کنترل دسترسی</span><strong>۹۳٪</strong></div><Progress value={93} /></div><div className="rounded-xl border border-[var(--ds-border)] bg-[var(--ds-muted)] p-3 text-sm leading-7 text-[var(--ds-foreground-soft)]"><Bot className="me-2 inline size-4 text-[var(--ds-accent)]" />تحلیل هوشمند در این نما صرفاً نمونه است و بدون تأیید انسانی اقدامی انجام نمی‌دهد.</div></CardContent></Card>
              </section>
