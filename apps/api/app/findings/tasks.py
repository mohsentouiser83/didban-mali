...
from app.findings.models import FindingGenerationRun, FindingRunStatus
from app.findings.service import execute_finding_generation
...
async def _mark_failed(run_id: UUID, company_id: UUID, actor_id: UUID, message: str) -> None:
    async with async_session_factory() as session:
        ...
        if run is not None:
            run.status = FindingRunStatus.FAILED
            run.completed_at = datetime.now(UTC)
            run.failure_code = "FINDING_GENERATION_FAILED"
...
async def _execute(
    ...
    try:
        async with async_session_factory() as session:
            await set_request_user(session, actor_id)
            await set_request_company(session, company_id)
            return await execute_finding_generation(
